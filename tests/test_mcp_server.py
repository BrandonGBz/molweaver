...
def test_mcp_server_name() -> None:
    ...
    assert mcp_instance.name == "pymol-figure-agent"
...
def test_four_resources_registered() -> None:
    ...
    expected = {
        "pymol-figure-agent://capabilities",
        "pymol-figure-agent://presets",
        "pymol-figure-agent://security-model",
        "pymol-figure-agent://examples",
    }
...
def test_capabilities_resource_returns_json() -> None:
    ...
    resources = mcp_instance._resource_manager.list_resources()
    caps = next(r for r in resources if str(r.uri) == "pymol-figure-agent://capabilities")
    result = caps.fn()
...
def test_presets_resource_returns_json() -> None:
    ...
    resources = mcp_instance._resource_manager.list_resources()
    presets = next(r for r in resources if str(r.uri) == "pymol-figure-agent://presets")
    result = presets.fn()
    data = json.loads(result)
